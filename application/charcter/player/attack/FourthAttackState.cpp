...
void FourthAttackState::Enter(Player* player) {
    // 攻撃状態への遷移時の初期化処理
...
void FourthAttackState::Update(Player* player) {
    UpdateJson();
    // 攻撃タイマーを更新
    ...
    if (attackTimer_ <= 0.0f) {
        if (isNextAttack_) {
            player->ChangeState(std::make_unique<FifthAttackState>());
        }
        else {
...
void FourthAttackState::Exit(Player* player) {
    // 攻撃状態の終了処理
    player->SetAttackFlag(false);
}

void FourthAttackState::UpdateJson()
{
    startPos_ = global_->GetVector3Value("fourthAttack", "startPos");
    midPos_ = global_->GetVector3Value("fourthAttack", "midPos");
    endPos_ = global_->GetVector3Value("fourthAttack", "endPos");
    startRot_ = global_->GetVector3Value("fourthAttack", "startRot");
    midRot_ = global_->GetVector3Value("fourthAttack", "midRot");
    endRot_ = global_->GetVector3Value("fourthAttack", "endRot");
}
